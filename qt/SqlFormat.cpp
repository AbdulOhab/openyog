...
const QSet<QString> kKeywords = {
    ...
    "by",     "having",     "order",      "limit",
    "offset", "join",       "inner",      "left",
    "right",  "outer",      "cross",      "on",
    "using",  "as",         "and",        "or",
    "not",    "in",         "is",         "null",
    "like",   "between",    "exists",     "union",
    "all",    "distinct",   "insert",     "into",
    "values", "update",     "set",        "delete",
    "create", "table",      "view",       "alter",
    "drop",   "add",        "column",     "primary",
    "key",    "foreign",    "references", "index",
    "unique", "constraint", "default",    "auto_increment",
    "engine", "charset",    "case",       "when",
    "then",   "else",       "end",        "asc",
    "desc",   "with",       "recursive",  "returning",
    "count",  "sum",        "avg",        "min",
    "max",    "true",       "false",      "if",
    "ifnull", "coalesce",
};
...
QList<Tok> lex(const QString &s)
{
    ...
    while(i < n) {
        ...
        if(c.isLetterOrNumber() || c == '_' || c == '$' || c == '.') {
            int j = i;
            while(j < n && (s[j].isLetterOrNumber() || s[j] == '_' || s[j] == '$' || s[j] == '.'))
                ++j;
...
QString formatOne(const QList<Tok> &toks)
{
    ...
    for(int t = 0; t < toks.size(); ++t) {
        ...
        if(!isWord && (w.startsWith(QStringLiteral("--")) || w.startsWith(QChar('#')) ||
        ...
        if(w == QStringLiteral(",")) {
            if(out.endsWith(' '))
                out.chop(1);
            out += w;
            out += (selectList && depth == 0) ? indent(1) : QStringLiteral(" ");
            continue;
        }
        if(w == QStringLiteral(";")) {
            if(out.endsWith(' '))
        ...
        if(isWord && depth == 0) {
            if(up == QStringLiteral("SELECT"))
                selectList = true;
            if(up == QStringLiteral("FROM"))
                selectList = false;
        }
        ...
        const bool freshLine =
            ...
            && !(up == QStringLiteral("BY")) && !out.endsWith(QStringLiteral("LEFT ")) &&
            !out.endsWith(QStringLiteral("RIGHT ")) && !out.endsWith(QStringLiteral("INNER ")) &&
            !out.endsWith(QStringLiteral("OUTER ")) && !out.endsWith(QStringLiteral("CROSS "));

        const bool andOr =
            isWord && depth == 0 && (up == QStringLiteral("AND") || up == QStringLiteral("OR"));
