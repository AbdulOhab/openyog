...
PostgresConnection::PostgresConnection(PGconn *conn) : m_conn(conn) {}
...
bool PostgresConnection::runBuffered(const QString &sql, DbResultSet *out, QString *error)
{
    ...
    if(st == PGRES_TUPLES_OK) {
        ...
        if(out) {
            ...
            for(int r = 0; r < nr; ++r) {
                QStringList row;
                for(int i = 0; i < nf; ++i)
                    row << (PQgetisnull(res, r, i) ? QStringLiteral("NULL")
    ...
    if(error)
        *error = QString::fromUtf8(PQresultErrorMessage(res));
    PQclear(res);
    return false;
}
...
bool PostgresConnection::streamQuery(
    ...
    const std::function<bool(const QVector<QByteArray> &fields, const QVector<bool> &isNull)>
        &onRow)
{
    const QByteArray utf8 = sql.toUtf8();
    if(!PQsendQuery(m_conn, utf8.constData())) {
        if(error)
            *error = QString::fromUtf8(PQerrorMessage(m_conn));
        return false;
    }
...
QByteArray PostgresConnection::escape(const QByteArray &raw)
{
    ...
    int errorFlag = 0;
    const size_t n =
        PQescapeStringConn(m_conn, out.data(), raw.constData(), size_t(raw.size()), &errorFlag);
    out.resize(int(n));
    return out;
}
...
QString PostgresConnection::quoteIdent(const QString &ident)
{
    ...
    const QString result =
        escaped ? QString::fromUtf8(escaped) : QLatin1Char('"') + ident + QLatin1Char('"');
    if(escaped)
        PQfreemem(escaped);
    return result;
}

QString PostgresConnection::lastError()
{
    return QString::fromUtf8(PQerrorMessage(m_conn));
}
qint64 PostgresConnection::affectedRows()
{
    return m_lastAffected;
}
...
QString PostgresConnection::info()
{
    return {};
...
QStringList PostgresConnection::listDatabases()
{
    DbResultSet rs;
    QStringList out;
    if(runBuffered(QStringLiteral("SELECT schema_name FROM information_schema.schemata "
...
QStringList PostgresConnection::listPhysicalDatabases()
{
    DbResultSet rs;
    QStringList out;
    if(runBuffered(QStringLiteral("SELECT datname FROM pg_database "
...
QStringList PostgresConnection::listTables(const QString &db, const QString &typeFilter)
{
    ...
    DbResultSet rs;
    QStringList out;
    if(runBuffered(QStringLiteral("SELECT table_name FROM information_schema.tables "
                                  "WHERE table_schema='%1' AND table_type='%2' ORDER BY table_name")
                       .arg(effectiveSchema(db), type),
                   &rs, nullptr))
        for(const QStringList &row : rs.rows)
            out << row.value(0);
    return out;
...
DbResultSet PostgresConnection::listColumns(const QString &db, const QString &table)
{
    ...
    for(const QStringList &row : cols.rows) {
        ...
        const bool isIdentity =
            row.value(4) == QStringLiteral("YES") || def.startsWith(QStringLiteral("nextval("));
        QStringList r;
        r << name << row.value(1)
...
DbResultSet PostgresConnection::listIndexes(const QString &db, const QString &table)
{
    ...
    for(const QStringList &row : rs.rows) {
        ...
        r << table << (isUnique ? QStringLiteral("0") : QStringLiteral("1"))
          << (isPk ? QStringLiteral("PRIMARY") : row.value(0)) << row.value(4) << row.value(3);
        out.rows << r;
    }
    return out;
}
...
QStringList PostgresConnection::listTriggers(const QString &db)
{
    DbResultSet rs;
    QStringList out;
    if(runBuffered(QStringLiteral("SELECT t.tgname FROM pg_trigger t "
                                  "JOIN pg_class c ON c.oid=t.tgrelid "
                                  "JOIN pg_namespace n ON n.oid=c.relnamespace "
                                  "WHERE NOT t.tgisinternal AND n.nspname='%1'")
                       .arg(effectiveSchema(db)),
                   &rs, nullptr))
        for(const QStringList &row : rs.rows)
            out << row.value(0);
    return out;
...
{
    DbResultSet rs;
    runBuffered(QStringLiteral("SELECT t.tgname, t.tgtype FROM pg_trigger t "
                               "JOIN pg_class c ON c.oid=t.tgrelid "
                               "JOIN pg_namespace n ON n.oid=c.relnamespace "
...
QString PostgresConnection::showCreate(const QString &kind, const QString &db, const QString &name,
                                       QString *error)
{
    ...
    if(k == QStringLiteral("VIEW")) {
        ...
        if(!runBuffered(QStringLiteral("SELECT pg_get_viewdef('%1.%2'::regclass, true)")
                            .arg(quoteIdent(schema), quoteIdent(name)),
                        &rs, error) ||
           rs.rows.isEmpty())
            return {};
    ...
    if(k == QStringLiteral("TRIGGER")) {
        DbResultSet rs;
        if(!runBuffered(
               QStringLiteral("SELECT pg_get_triggerdef(t.oid) FROM pg_trigger t "
                              "JOIN pg_class c ON c.oid=t.tgrelid "
                              "JOIN pg_namespace n ON n.oid=c.relnamespace "
           ...
           rs.rows.isEmpty()) {
            if(error && error->isEmpty())
                *error = QStringLiteral("trigger not found");
            return {};
        }
    ...
    if(k == QStringLiteral("PROCEDURE") || k == QStringLiteral("FUNCTION")) {
        ...
        if(!runBuffered(QStringLiteral("SELECT pg_get_functiondef(p.oid) FROM pg_proc p "
           ...
           rs.rows.isEmpty()) {
            if(error && error->isEmpty())
                *error = QStringLiteral("routine not found");
            return {};
        }
        return rs.rows.first().value(0);
    }
    if(k == QStringLiteral("TABLE")) {
        ...
        if(cols.rows.isEmpty()) {
            if(error)
                *error = QStringLiteral("table not found");
            return {};
        }
        QStringList lines;
        QStringList pkCols;
    ...
    if(error)
        *error = QStringLiteral("not supported on PostgreSQL");
    return {};
}
