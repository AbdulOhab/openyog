...
QString delimited(const QStringList &headers,
                  const std::function<QString(int, int)> &cell, int rows, int cols,
                  const Options &opt, QChar sep)
{
        ...
        for(int r = 0; r < rows; ++r) {
            ...
            for(int c = 0; c < cols; ++c) {
                ...
                o.insert(headers.value(c, QString::number(c)),
                         isNull(v, opt) ? QJsonValue(QJsonValue::Null)
                                        : QJsonValue(v));
            }
            arr.append(o);
        }
...
Format formatForName(const QString &name)
{
    ...
    if(n.startsWith(QStringLiteral("TSV")))       return Format::Tsv;
    if(n.startsWith(QStringLiteral("HTML")))      return Format::Html;
    if(n.startsWith(QStringLiteral("JSON")))      return Format::Json;
    if(n.startsWith(QStringLiteral("MARKDOWN")))  return Format::Markdown;
    if(n.startsWith(QStringLiteral("XML")))       return Format::Xml;
    if(n.startsWith(QStringLiteral("SQL")))       return Format::Sql;
    if(n.startsWith(QStringLiteral("EXCEL")))     return Format::Excel;
    return Format::Csv;
}
...
QString suffixFor(Format f)
{
    ...
    case Format::Sql:      return QStringLiteral("sql");
    case Format::Excel:    return QStringLiteral("xls");
    case Format::Csv:      break;
    }
